...
function buildMatrixNetworkFields(params: {
  ...
  const proxyDispatcherPolicy = params.proxy
    ? ({ mode: "explicit-proxy", proxyUrl: params.proxy } satisfies PinnedDispatcherPolicy)
    : undefined;
  const dispatcherPolicy = params.dispatcherPolicy ?? proxyDispatcherPolicy;
