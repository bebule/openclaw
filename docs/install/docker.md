...
<Steps>
  ...
  <Step title="Open the Control UI">
    ...
    `docker-compose.yml` publishes `18789` and `18790` to loopback by default
    (`127.0.0.1`) even when `OPENCLAW_GATEWAY_BIND=lan` inside the container, so
    the host exposure stays local unless you explicitly change the published
    host mapping.
...
`OPENCLAW_BRIDGE_PORT=18790`, and apply the firewall guidance in
[Security hardening for network exposure](/gateway/security).
